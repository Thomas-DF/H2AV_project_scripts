...
######################################################################################################################################################
######################################################################################################################################################
## This Script provides commmand lines with tools versions that have bene used to pre-process RNA-seq data from fastq to bigwig and profile matrices.
...
######################################################################################################################################################
######################################################################################################################################################
## This Script provides commmand lines with tools versions that have bene used to pre-process DRIP-seq data from fastq to bigwig and profile matrices.
...
trim_galore -q 20 -stringency 5 --fastqc_args "--nogroup --outdir ./fastqc/trimmed_fqc" -o ./fastq/trimmed_fq ./fastq/dripseq_file.fq.gz

######################################################################################################################################################
## READS INDEX
######################################################################################################################################################
#  bwa-0.7.15 	
bwa aln -t 8 ./REF_GENOME/dmel-all-chromosome-r6.13.fasta ./fastq/trimmed_fq/dripseq_file_trimmed.fq > ./SAI/dripseq_file_trimmed.sai
...
bwa samse ./REF_GENOME/dmel-all-chromosome-r6.13.fasta ./SAI/chipseq_file_trimmed.sai ./fastq/trimmed_fq/dripseq_file_trimmed.fq > ./SAM/dripseq_file_trimmed.sam
...
grep 'XT:A:U' ./SAM/dripseq_file_trimmed.sam > ./SAM/dripseq_file_trimmed.uniqsam

samtools view -bS -F 1540 -q 30 ./SAM/dripseq_file_trimmed_filt.sam -o ./BAM_filt/dripseq_file_trimmed_filt.bam
...
samtools sort ./BAM_filt/dripseq_file_trimmed_filt.bam -o ./BAM_sort/dripseq_file_trimmed_filt_sort.bam
...
bamCoverage --bam ./BAM_sort/dripseq_file_trimmed_filt_sort.bam --binSize 10 --normalizeTo1x 125464728 -o ./BIGWIG/dripseq_file_trimmed_filt_sort_RPGC.bw
...
computeMatrix scale-regions -S ./BIGWIG/dripseq_file_trimmed_filt_sort_RPGC.bw -R ./REF_GENOME/TxDb.Dmelanogaster.Ensembl.dm6.bed -bs 10 -m 4000 -a 2000 -b 4000 -out ./PROFIL_MATRIX/GB/dripseq_file_trimmed_filt_sort_RPGC.out --outFileNameMatrix ./PROFIL_MATRIX/GB/dripseq_file_trimmed_filt_sort_RPGC.outmatrix
...
macs2 callpeak -t BAM/dripseq_file_trimmed_filt_sort.bam -f BAM -g dm -n file_name --nomodel --extsize 147 --outdir macs2/
