###################################################################################
# From 2 or 3 character vectors, plot venn + enrichment test fisher + SuperExactTest
#
###################################################################################
...
## On genotoul Vennerable
# library(gplots)
# library(Vennerable)
# library(SuperExactTest)

plotVENNerable = function(data_to_venn, Nb_REF, doWeights_venn=TRUE, fisher_side = "greater", outdir){
	vennNAME = paste(names(data_to_venn), collapse="_")
	Venn_ovlp = Venn(data_to_venn)
	print(Venn_ovlp)
	color_Venn = VennThemes(compute.Venn(Venn_ovlp, doWeights = doWeights_venn))
  ...
  # color_Venn[["Face"]][["011-1"]]$fill = "#721e1e"
	...
	#fisher exact test
	fishertest = fisher_namesList(data_to_venn,data_to_venn,Nb_REF, fisher_side = fisher_side)
	...
	########### PLOT #########################
	pdf(paste0(outdir, "PLOTvenn_",vennNAME,".pdf"), width= 20)
	plot(Venn_ovlp, gp = color_Venn, doWeights = doWeights_venn)
	par(mfrow=c(2,1))
	textplot(paste0("fisher test alternative = ", fisher_side))
	textplot(fishertest$p.value,  valign="top")
 	textplot("odds.ratio")
	textplot(fishertest$odds.ratio)
	textplot("Super exact test 3ple enrich")
	textplot(SupetTestToPrint)
	plot(Venn_ovlp, gp = color_Venn, doWeights = doWeights_venn, show = list(SetLabels = FALSE, FaceText = ""))
	dev.off()
...
fisher_namesList=function(list1, list2, total, effMax = F, fisher_side = "greater")
 {
 ...
 for (i in 1:length(list1)){
   liste1=list1[[i]]
    for (j in 1:length(list2)){
      ...
      m_pval[i,j]=signif(fisher.test(matrix(c(inter,length(liste1)-inter,length(liste2)-inter,max(total-length(liste1)-length(liste2)+inter,0)),ncol=2),alternative=fisher_side)$p.value,2)
      m_or[i,j]=signif(fisher.test(matrix(c(inter,length(liste1)-inter,length(liste2)-inter,max(total-length(liste1)-length(liste2)+inter,0)),ncol=2),alternative=fisher_side)$estimate,2)
    }
 }
